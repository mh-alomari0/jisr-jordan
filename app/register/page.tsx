...
export default function RegisterPage() {
  ...
  const handleRegister = async (
    ...
    if (password.length < 8) {
      setError(
        "يجب أن تكون كلمة المرور من 8 خانات على الأقل.",
      );
      return;
    }
    ...
    try {
      ...
      if (!result.success) {
        setError(
          result.error || "تعذر إكمال التسجيل.",
        );
        setLoading(false);
        return;
      }
    ...
    <main className="mx-auto my-5 grid min-h-[720px] max-w-6xl overflow-hidden rounded-[2rem] border border-theme bg-surface shadow-soft lg:grid-cols-2">
      ...
      <section className="flex items-center p-5 sm:p-9 lg:p-12">
        <div className="mx-auto w-full max-w-md">
          <Link
            href="/"
            className="mb-7 inline-flex items-center gap-2 text-sm font-bold lg:hidden"
          >
          ...
          <p className="mt-2 text-xs leading-6 text-muted">
            حساب واحد للحجوزات والرسائل والمفضلة وعروض
            الأسعار.
          </p>
          ...
          {success ? (
            ...
            >
              {error && (
                <div
                  role="alert"
                  className="flex gap-2 rounded-2xl bg-[rgb(var(--danger)/0.08)] p-4 text-xs text-[rgb(var(--danger))]"
                >
