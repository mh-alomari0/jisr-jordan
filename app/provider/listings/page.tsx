...
export default async function ProviderListingsPage() {
  const [listingsResult, categoriesResult, taxonomyResult] = await Promise.all([getProviderListingsAction(), getMarketplaceCategoriesAction(), getHomeServiceTaxonomyAction()]);
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 p-3 sm:p-6">
      ...
      <ProviderListingsClient listings={listingsResult.listings} categories={categoriesResult.categories || []} serviceTypes={(taxonomyResult.categories || []).flatMap((category) => category.serviceTypes)} />
    </div>
  );
}
